...
class RandomTranslateWithReflect:
    ...
    随机翻译图像
    垂直和水平平移n个像素，其中n是从[-max_translation，max_translation]为每个轴独立均匀绘制的整数。
    用反射衬垫填充未覆盖的空白区域。
    """
...
def relabel_dataset(dataset, labels):
    ...
    for idx in range(len(dataset.label_paths)):
        path, _ = dataset.label_paths[idx]
        filename = os.path.basename(path)
        if filename in labels:
            label_idx = dataset.class_to_idx[labels[filename]]
            dataset.label_paths[idx] = path, label_idx
            del labels[filename]
    for idx in range(len(dataset.unlabel_paths)):
        path, _ = dataset.unlabel_paths[idx]
        dataset.unlabel_paths[idx] = path, NO_LABEL
